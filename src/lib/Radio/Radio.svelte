...
<label class="radio-container {classes ?? ''}" class:disabled data-pw={testId}>
  <input
    type="radio"
    class="radio-input"
    {name}
...
<style>
  ...
  .radio-indicator {
    ...
    border: var(--radio-border, 2px solid currentColor);
  ...
  .radio-indicator.checked {
    border: var(--radio-selected-border, 2px solid currentColor);
    background-color: var(--radio-selected-background, #ffffff);
  }

  .radio-indicator.disabled {
    border: var(--radio-disabled-border, 2px solid currentColor);
    background-color: var(--radio-disabled-background, transparent);
  }

  .radio-container:not(.disabled):hover .radio-indicator:not(.checked) {
    border: var(--radio-hover-border, 2px solid currentColor);
  }

  .radio-input:focus-visible + .radio-indicator {
    box-shadow: var(--radio-focus-shadow, 0 0 0 3px currentColor);
  }
  ...
  .radio-dot {
    ...
    background-color: var(--radio-dot-color, currentColor);
    transition: var(--radio-transition, 0.2s);
  }
  ...
  .radio-dot.checked.disabled {
    background-color: var(--radio-disabled-dot-color, currentColor);
  }

  .radio-text {
    font-size: var(--radio-text-font-size, 14px);
    font-weight: var(--radio-text-font-weight, 400);
    color: var(--radio-text-color, currentColor);
  }

  .radio-text.disabled {
    color: var(--radio-disabled-text-color, currentColor);
  }
</style>
